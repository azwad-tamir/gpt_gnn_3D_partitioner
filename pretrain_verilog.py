...
def prepare_data(pool):
    ...
    for _ in np.arange(args.n_batch - 1):
        jobs.append(pool.apply_async(GPT_sample,
                                     args=(randint(), pre_target_nodes, {1: True}, args.batch_size, feature_reddit)))
    jobs.append(
        pool.apply_async(GPT_sample, args=(randint(), train_target_nodes, {1: True}, args.batch_size, feature_reddit)))
    return jobs
        ...
        for placement_type in placement_types:
            for timing_type in timing_types:
                for hop_type in hop_types:
                    ...
                    parser.add_argument('--data_dir', type=str, default='graphs/graph_'+subject+'_'+str(timing)+'_util'+str(util)+'_placement_'+placement_type+'_hop_'+hop_type+'_timing_'+timing_type+'.pk', help='The address of preprocessed graph.')
                    # parser.add_argument('--pretrain_model_dir', type=str, default='built_models/dhm_mini_gpt_all_reddit',
                    #                     help='The address for storing the pre-trained models.')
                    try:
                        os.makedirs('built_models/')
                    except OSError as error:
                        print(error)
                    parser.add_argument('--pretrain_model_dir', type=str, default='built_models/model_'+subject+'_'+str(timing)+'_util'+str(util)+'_placement_'+placement_type+'_hop_'+hop_type+'_timing_'+timing_type,
                                        help='The address for storing the pre-trained models.')
